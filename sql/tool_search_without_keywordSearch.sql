...
-- tool search with getting all description full and short -- without keyword search
	...
	JOIN (
		...
		SELECT
			...
			CONCAT(
				...
				COALESCE(CONCAT(min_torque_rating, 'min ft-lb'), ''), ' ',
				COALESCE(CONCAT(max_torque_rating, 'max ft-lb'), ''), ' ',
				COALESCE(CONCAT(volt_rating, 'V'), ''), ' ',
				COALESCE(CONCAT(amp_rating, 'Amp'), ''), ' ',
				COALESCE(CONCAT(min_rpm_rating, 'min rpm'), ''), ' ',
				COALESCE(CONCAT(max_rpm_rating, 'max rpm'), '')
